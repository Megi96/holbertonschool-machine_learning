#!/usr/bin/env python3
"""Deep Neural Network for multiclass classification with selectable hidden activation"""
...
class DeepNeuralNetwork:
    ...
    @property
    def activation(self):
        """Getter for private __activation attribute"""
    ...
    def forward_prop(self, X):
        ...
        for i in range(self.L):
            ...
            if i == self.L - 1:  # output layer uses softmax
                self.cache["A{}".format(i + 1)] = self.softmax(Z)
            else:  # hidden layers use chosen activation
                if self.__activation == 'sig':
                    self.cache["A{}".format(i + 1)] = self.sigmoid(Z)
                else:
                    self.cache["A{}".format(i + 1)] = self.tanh(Z)
