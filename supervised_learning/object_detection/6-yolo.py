...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for i, box in enumerate(boxes):
            ...
            # Draw bounding box in blue, thickness 2
            cv2.rectangle(
                annotated,
                (x1, y1),
                (x2, y2),
            ...
            # Build label: class name and score rounded to 2 decimal places
            class_name = self.class_names[box_classes[i]]
            score = round(float(box_scores[i]), 2)
            label = "{} {:.2f}".format(class_name, score)

            # Draw label in red, 5 pixels above top-left corner of box
            cv2.putText(
                annotated,
                label,
                (x1, y1 - 5),
